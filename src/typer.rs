...
impl Typer {
    ...
    pub fn type_delta(&mut self, text: &str) -> Result<()> {
        for ch in text.chars().flat_map(normalize) {
            ...
            if shift {
                self.dev.emit(&[*KeyEvent::new(KeyCode::KEY_LEFTSHIFT, 1)])?;
            }
            self.dev.emit(&[*KeyEvent::new(code, 1)])?;
            self.dev.emit(&[*KeyEvent::new(code, 0)])?;
            if shift {
                self.dev.emit(&[*KeyEvent::new(KeyCode::KEY_LEFTSHIFT, 0)])?;
            }
            sleep(KEY_DELAY);
        }
        Ok(())
...
const ALL_KEYS: &[KeyCode] = &[
    ...
    KeyCode::KEY_Z, KeyCode::KEY_1, KeyCode::KEY_2, KeyCode::KEY_3, KeyCode::KEY_4,
    KeyCode::KEY_5, KeyCode::KEY_6, KeyCode::KEY_7, KeyCode::KEY_8, KeyCode::KEY_9,
    KeyCode::KEY_0, KeyCode::KEY_SPACE, KeyCode::KEY_ENTER, KeyCode::KEY_TAB,
    ...
    KeyCode::KEY_COMMA, KeyCode::KEY_DOT, KeyCode::KEY_SLASH, KeyCode::KEY_GRAVE,
...
mod tests {
    ...
    fn normalize_typographic() {
        let s: String = "\u{201C}hi\u{201D}\u{2014}ok\u{2026}".chars().flat_map(normalize).collect();
        assert_eq!(s, "\"hi\"-ok...");
    }
}
